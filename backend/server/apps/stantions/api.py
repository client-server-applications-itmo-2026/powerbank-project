...
def pull_stantion_task(request, data: PullStantionTaskRequest) -> StantionTaskInfo:
    """
    Метод для получения задач станцией. Станция шлет запрос, а мы ей в ответ отдаем задачу, которую она должна выполнить.
    """
...
def create_stantion_task_result(request, data: CreateStantionTaskResultRequest):
    """
    Метод для создания результата выполнения задачи станцией. Станция шлет запрос с id задачи, которую она выполнила, и результатом выполнения (успех или ошибка), а мы сохраняем этот результат в базе данных и меняем статус задачи на "выполнена". Если задача не найдена или не была принята этой станцией, то возвращаем ошибку.
    """
    return StantionTaskInfo.from_db_instance(
...
def register_stantion(request, data: RegisterStantionRequest):
    """
    Метод для регистрации новой станции. Станция шлет запрос с информацией о себе (id, модель, местоположение), а мы создаем запись о ней в базе данных. Если станция с таким id уже существует, то возвращаем ошибку.
    """
...
def retrieve_stantions(
    ...
    """
    Метод для получения списка станций, отсортированных по удаленности от переданной точки. Станция шлет запрос с координатами точки, а мы возвращаем список станций, отсортированных по удаленности от этой точки. Также в ответе мы указываем количество свободных слотов и количество доступных батареек в каждой станции.
    """
