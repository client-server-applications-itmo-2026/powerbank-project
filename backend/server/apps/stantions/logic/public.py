...
def create_release_battery_task(
    ...
    if last_heartbeat is None or (
        timezone.now() - last_heartbeat.created_at
    ) > datetime.timedelta(minutes=10):
        raise DomainError("Stantion is not active")

    if battery_instance is None:
        slot_instance = last_heartbeat.slot_states.filter(state=SlotStateEnum.CHARGED).first()  # type: ignore
        if slot_instance is None:
            raise DomainError("No charged batteries available at the moment")
        battery_instance = slot_instance.registered_battery

    if not last_heartbeat.slot_states.filter(
